...
$status_labels = [
	...
	'approved' => __('Zaakceptowane', 'studio-a7-odstap'),
	'rejected' => __('Odrzucone', 'studio-a7-odstap'),
	'cancelled' => __('Anulowane', 'studio-a7-odstap'),
];
...
<div class="wrap a7w-admin-wrap">
	<?php if (isset($_GET['a7w_decision'])): // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>
		<div
			class="notice <?php echo 'success' === sanitize_key(wp_unslash($_GET['a7w_decision'])) ? 'notice-success' : 'notice-error'; ?> is-dismissible">
			<p><?php echo 'success' === sanitize_key(wp_unslash($_GET['a7w_decision'])) ? esc_html__('Decyzja została zapisana.', 'studio-a7-odstap') : esc_html__('Nie udało się zapisać decyzji.', 'studio-a7-odstap'); ?>
			</p>
		</div>
	<?php endif; ?>
	<?php if (isset($_GET['a7w_action_error'])): // phpcs:ignore WordPress.Security.NonceVerification.Recommended ?>
		<div class="notice notice-warning is-dismissible">
			<p><?php echo esc_html(rawurldecode(sanitize_text_field(wp_unslash($_GET['a7w_action_error'])))); ?></p>
		</div><?php endif; ?>
	...
	<form method="get" class="a7w-search-form">
		...
		<input type="search" name="s" class="a7w-search-input" value="<?php echo esc_attr($search); ?>"
		...
		<table class="a7w-table widefat striped">
			<thead>
				<tr>
					...
					<th><?php esc_html_e('Decyzja i audyt', 'studio-a7-odstap'); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php foreach ($items as $row):
					...
					$status_class = [
						...
						'approved' => 'a7w-status-confirmed',
						'rejected' => 'a7w-status-rejected',
						'cancelled' => 'a7w-status-rejected',
					];
					$status_text = [
					...
					$audit_entries = json_decode((string) $row->audit_log, true);
					$audit_entries = is_array($audit_entries) ? $audit_entries : array();
					?>
					<tr>
						<th class="check-column"><input type="checkbox" disabled></th>
						...
						<td>
							<?php if ('confirmed' === $row->status): ?>
								<form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
									<input type="hidden" name="action" value="a7w_decide_withdrawal"><input type="hidden" name="withdrawal_id" value="<?php echo esc_attr($row->id); ?>">
									<?php wp_nonce_field('a7w_decide_withdrawal_' . $row->id); ?>
									<textarea name="admin_note" rows="2" class="widefat" placeholder="<?php esc_attr_e('Notatka dla klienta (opcjonalna)', 'studio-a7-odstap'); ?>"></textarea>
									<p><button type="submit" name="decision" value="approved" class="button button-primary"><?php esc_html_e('Zatwierdź', 'studio-a7-odstap'); ?></button> <button type="submit" name="decision" value="rejected" class="button"><?php esc_html_e('Odrzuć', 'studio-a7-odstap'); ?></button></p>
								</form>
							<?php elseif (!empty($row->admin_note)): ?><p><?php echo nl2br(esc_html($row->admin_note)); ?></p><?php endif; ?>
							<?php if ($audit_entries): ?><details><summary><?php esc_html_e('Dziennik audytu', 'studio-a7-odstap'); ?></summary><ul><?php foreach ($audit_entries as $entry): ?><li><?php echo esc_html(sprintf('%s — %s%s', $entry['at'] ?? '', $entry['event'] ?? '', empty($entry['note']) ? '' : ': ' . $entry['note'])); ?></li><?php endforeach; ?></ul></details><?php endif; ?>
						</td>
					</tr>
				<?php endforeach; ?>
			</tbody>
