...
<div class="a7w-modal-overlay" id="a7w-modal-<?php echo esc_attr( $order_id ); ?>" role="dialog" aria-modal="true" aria-labelledby="a7w-modal-title-<?php echo esc_attr( $order_id ); ?>" hidden>
	<div class="a7w-modal">
		...
		<div class="a7w-modal__header">
			...
			<button type="button" class="a7w-modal__close a7w-close-modal" aria-label="<?php esc_attr_e( 'Zamknij', 'studio-a7-odstap' ); ?>">
		...
		<div class="a7w-modal__body a7w-step-panel" data-panel="1">
			<p class="a7w-modal__intro">
				...
				printf(
					...
					esc_html__( 'Składasz oświadczenie o odstąpieniu od umowy sprzedaży dotyczącej zamówienia nr %s. Prawo do odstąpienia przysługuje Ci w terminie 14 dni od otrzymania towaru.', 'studio-a7-odstap' ),
			...
			<form class="a7w-form" id="a7w-form-step1-<?php echo esc_attr( $order_id ); ?>" novalidate>
				...
				<div class="a7w-form__group">
					...
					<input
						type="text"
						id="a7w-name-<?php echo esc_attr( $order_id ); ?>"
				...
				<div class="a7w-form__group">
					...
					<input
						type="email"
						id="a7w-email-<?php echo esc_attr( $order_id ); ?>"
						class="a7w-form__input"
				...
				<div class="a7w-form__group">
					<label class="a7w-form__label" for="a7w-reason-<?php echo esc_attr( $order_id ); ?>">
						...
						<?php if ( ! $require_reason ) : ?>
							<span class="a7w-form__optional"><?php esc_html_e( '(opcjonalnie)', 'studio-a7-odstap' ); ?></span>
						<?php endif; ?>
					</label>
					<textarea
						id="a7w-reason-<?php echo esc_attr( $order_id ); ?>"
						name="reason"
						class="a7w-form__textarea"
						rows="3"
						placeholder="<?php esc_attr_e( 'Np. produkt nie spełnił moich oczekiwań…', 'studio-a7-odstap' ); ?>"
						<?php echo $require_reason ? 'required' : ''; ?>
					></textarea>
				</div>
				...
				<div class="a7w-form__group a7w-form__group--checkbox">
					<label class="a7w-checkbox" for="a7w-consent-<?php echo esc_attr( $order_id ); ?>">
						<input
							type="checkbox"
							id="a7w-consent-<?php echo esc_attr( $order_id ); ?>"
							name="consent"
							required
							class="a7w-checkbox__input"
						>
				...
				<!-- Komunikat błędu -->
				<div class="a7w-form__error" id="a7w-error-step1-<?php echo esc_attr( $order_id ); ?>" role="alert" hidden></div>
				...
				<div class="a7w-form__actions">
					...
					<button type="submit" class="a7w-btn a7w-btn--primary" id="a7w-submit-step1-<?php echo esc_attr( $order_id ); ?>">
		...
		<div class="a7w-modal__body a7w-step-panel" data-panel="2" hidden>
			...
			<!-- Komunikat błędu -->
			<div class="a7w-form__error" id="a7w-error-step2-<?php echo esc_attr( $order_id ); ?>" role="alert" hidden></div>

			<!-- Przyciski -->
			<div class="a7w-form__actions">
				<button type="button" class="a7w-btn a7w-btn--ghost a7w-back-to-step1" data-order-id="<?php echo esc_attr( $order_id ); ?>">
					<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
						<polyline points="15 18 9 12 15 6"/>
					</svg>
					<?php esc_html_e( 'Wróć', 'studio-a7-odstap' ); ?>
				</button>
				<button type="button" class="a7w-btn a7w-btn--danger a7w-confirm-step2" data-order-id="<?php echo esc_attr( $order_id ); ?>">
					<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
